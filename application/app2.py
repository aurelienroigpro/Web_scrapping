...
# Normalisation simple en 2 catégories
tb = df["type_bien"].astype(str).str.lower()
df_nat = df.copy()
...
# On garde uniquement Appart/Maison pour la comparaison
...
if df_am.empty:
    ...
    # Donut = part des annonces
    with c1:
        st.caption("Part des annonces")
        parts = df_am["type_cat"].value_counts().reset_index()
        parts.columns = ["type_cat", "annonces"]
        st.plotly_chart(
            px.pie(parts, names="type_cat", values="annonces", hole=0.4),
            use_container_width=True
        )

    # 3 mini bar charts = comparaison des médianes (lisible)
    with c2:
...
fig.update_layout(
    xaxis=dict(range=[0, 350], title="Surface (m²)"),
